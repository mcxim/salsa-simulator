...
def make_graph(graph):
    ...
    tension_on_one = MoveNode("Tension", 2)
    botala = MoveNode("Botala", 6)
    enchufala = MoveNode("Enchufala", 2)
    doble_of_enchufala = MoveNode("Doble", 4)
    complicado_of_enchufala = MoveNode("Complicado", 4)
    dile_que_no = MoveNode("Dile Que No", 2)
    dile_que_pare = MoveNode("Dile Que Pare", 6)
    ...
    guapea.add_signal(1, "Tension", 0, tension_on_one)
    tension_on_one.add_signal("Raise hand", 0, enchufala)
    tension_on_one.add_signal("Keep hand at same level", 0, botala).resolves_to(closed)
    ...
    closed.add_signal(7, "Turn partner to the left and raise hand", 2, vamos_abajo).resolves_to(closed)
    dile_que_no.add_signal("Make way", 6, guapea)
    dile_que_no.add_signal("Push partner backwards", 0, dile_que_pare).resolves_to(closed)
